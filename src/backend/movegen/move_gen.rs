...
pub fn moves(state: &mut State) -> Vec<Moove> {
    ...
    let friendly_pieces_bb = state.bb_mngr.get_all_pieces_bb_off(active_side);
    let enemy_pieces_bb = state.bb_mngr.get_all_pieces_bb_off(active_side.oppo());
    ...
    let mut moves = Vec::with_capacity(EXPECTED_MOVE_CAPACITY);
