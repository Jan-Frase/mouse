...
use mouse::State;
use mouse::backend::movegen::move_gen::MoveGenerator;
...
pub fn criterion_make_unmake_move(c: &mut Criterion) {
    ...
    let moves = move_generator.generate_moves(&mut state);
...
pub fn criterion_move_gen(c: &mut Criterion) {
    ...
    group.bench_function("Move gen", |b| {
        b.iter(|| {
            let move_generator = MoveGenerator::new();
            let _ = move_generator.generate_moves(std::hint::black_box(&mut state));
        })
    });
    group.finish();
}
